	...
	if [[ $msg == "."* ]]; then
		...
		# Count words with detailed info
			...
			if [[ $word == "all" ]]; then
				...
				shopt -u nocasematch
				gnuplot -e "set terminal png tiny size '768x480';set title 'Stats for onodera~';set format x '%Y-%m-%d';set xdata time;set timefmt '%Y-%m-%d';set xrange [ '2014-09-14' : '$(date +"%Y-%m-%d")' ];set style fill pattern 1 border;set boxwidth 23*3600 absolute;plot '$HOME/.punyama/count.txt' using 2:1 with boxes notitle lt -1;" > "$HOME/.punyama/graph.png"
				...
				shopt -u nocasematch
				gnuplot -e "set terminal png tiny size '768x480';set title 'Stats for $word1 and $word2~';set format x '%Y-%m-%d';set xdata time;set timefmt '%Y-%m-%d';set xrange [ '2014-09-14' : '$(date +"%Y-%m-%d")' ];set style fill pattern 1 border;set boxwidth 23*3600 absolute;plot '$HOME/.punyama/count.txt' using 3:(\$1+\$2) with boxes title '$word1' lt -1,'' using 3:1 with boxes title '$word2' lt -1;" > "$HOME/.punyama/graph.png"
